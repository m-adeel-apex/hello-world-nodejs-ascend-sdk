 ...
 */
export async function createOrderCreateOrder(
  ...
  >
> {
  const input: operations.OrderServiceCreateOrderRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.OrderServiceCreateOrderRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.OrderCreate, { explode: true });
  ...
  const path = pathToFunc("/trading/v1/accounts/{account_id}/orders")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.OrderServiceCreateOrderResponse$inboundSchema, {
      key: "Order",
    }),
    ...
    M.json(
      "default",
      operations.OrderServiceCreateOrderResponse$inboundSchema,
      { key: "Status" },
    ),
