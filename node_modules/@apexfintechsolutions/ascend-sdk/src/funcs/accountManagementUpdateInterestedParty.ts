 ...
 */
export async function accountManagementUpdateInterestedParty(
  ...
  >
> {
  const input: operations.AccountsUpdateInterestedPartyRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.AccountsUpdateInterestedPartyRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.InterestedPartyUpdate, {
    explode: true,
  });
  ...
  >(
    M.json(
      200,
      operations.AccountsUpdateInterestedPartyResponse$inboundSchema,
      { key: "InterestedParty" },
    ),
