...
exports.createOrderCreateOrder = createOrderCreateOrder;
 ...
 */
async function createOrderCreateOrder(client, orderCreate, accountId, options) {
    const input = {
        orderCreate: orderCreate,
        accountId: accountId,
    };
    const parsed = (0, schemas_js_1.safeParse)(input, (value) => operations.OrderServiceCreateOrderRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
        return parsed;
    }
    const payload = parsed.value;
    const body = (0, encodings_js_1.encodeJSON)("body", payload.OrderCreate, { explode: true });
    const pathParams = {
    ...
    const path = (0, url_js_1.pathToFunc)("/trading/v1/accounts/{account_id}/orders")(pathParams);
    ...
    const [result] = await M.match(M.json(200, operations.OrderServiceCreateOrderResponse$inboundSchema, {
        key: "Order",
    }), M.jsonErr([400, 401, 403, 409, 500, 503], errors.Status$inboundSchema), M.fail(["4XX", "5XX"]), M.json("default", operations.OrderServiceCreateOrderResponse$inboundSchema, { key: "Status" }))(response, req, { extraFields: responseFields });
    if (!result.ok) {
        return result;
    }
    return result;
}
//# sourceMappingURL=createOrderCreateOrder.js.map