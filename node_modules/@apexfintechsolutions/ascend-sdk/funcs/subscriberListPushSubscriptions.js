 ...
 */
async function subscriberListPushSubscriptions(client, filter, pageSize, pageToken, options) {
    const input = {
        filter: filter,
        pageSize: pageSize,
        pageToken: pageToken,
    ...
    const [result] = await M.match(M.json(200, operations.SubscriberListPushSubscriptionsResponse$inboundSchema, { key: "ListPushSubscriptionsResponse" }), M.jsonErr([400, 401, 403, 500], errors.Status$inboundSchema), M.fail(["4XX", "5XX"]), M.json("default", operations.SubscriberListPushSubscriptionsResponse$inboundSchema, { key: "Status" }))(response, req, { extraFields: responseFields });
    if (!result.ok) {
        return result;
    }
    return result;
