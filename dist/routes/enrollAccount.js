...
router.get("/", (_req, res) => {
    res.sendFile(path_1.default.join(__dirname, '..', 'templates', 'enrollAccount.html'));
});
router.post("/", async (req, res) => {
    var _a;
    try {
        const { accountId, principalApproverId } = req.body;
        if (!accountId) {
            res.status(400).json({
                success: false,
        ...
        const result = await ascend_1.default.enrollmentsAndAgreements.enrollAccount({
            enrollment: {
                principalApproverId: principalApproverId,
                type: components_1.EnrollmentCreateType.RegistrationIndividual,
            },
        }, formattedAccountId);
...
//# sourceMappingURL=enrollAccount.js.map